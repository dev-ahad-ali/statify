...
function signatureBase(
  ...
  const lines = components.map((component) => {
    if (component === "@method")
      return `"@method": ${request.method.toUpperCase()}`;
  ...
  lines.push(
    `"@signature-params": (${components.map((component) => `"${component}"`).join(" ")});${params}`,
  );
  return lines.join("\n");
}
...
async function directory(
  ...
  if (cached)
    return JSON.parse(cached) as {
      ...
      publicKeys?: Array<Record<string, unknown>>;
    };
  const response = await fetchFn(
    `${origin}/.well-known/http-message-signatures-directory`,
  );
  if (!response.ok)
    throw new Error(`Web Bot Auth directory returned ${response.status}`);
...
export async function verifyWebBotAuth(
  ...
  if (!request.signature || !request.signatureInput || !request.signatureAgent)
  ...
  try {
    ...
    const signature = signatureValue(
      request.signature,
      request.signatureInput.split("=")[0]!,
    );
    ...
    const key = keys.find(
      (item) => item.kid === parsed.keyId || item.keyid === parsed.keyId,
    );
    if (
      !key ||
      key.kty !== "OKP" ||
    ...
    const valid = await crypto.subtle.verify(
      ...
      new TextEncoder().encode(
        signatureBase(request, parsed.components, parsed.params),
      ),
    );
    if (!valid) return { verified: false };
    const hostname = new URL(agentOrigin).hostname;
    const vendor =
      Object.entries(vendors).find(
    ...
    return {
      ...
      vendor,
      keyId: parsed.keyId,
      model: headerValue(request.headers, "x-agent-model"),
    };
  } catch {
    return { verified: false };
  }
