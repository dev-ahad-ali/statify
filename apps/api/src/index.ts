...
export async function scheduled(
  ...
  if (controller.cron === "0 4 * * SUN") {
    ctx.waitUntil(
      deleteExpiredEvents(runtimeEnv.DB).catch((error) =>
        console.error("event retention failed", error),
      ),
    );
    return;
  }
  ...
  ctx.waitUntil(
    runNightlyPsi(new AuditsRepository(runtimeEnv.DB), runtimeEnv.PSI_API_KEY),
  );
}
