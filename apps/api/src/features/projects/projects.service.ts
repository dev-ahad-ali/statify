...
function normalizeDomain(value: string) {
  ...
  if (!DOMAIN_PATTERN.test(domain))
    throw new ProjectValidationError(`Invalid domain: ${value}`);
  return domain;
}
...
function cacheValue(project: ProjectRow): ProjectCache {
  return {
    ...
    domain: project.domain,
    allowedDomains: parseAllowedDomains(project.allowed_domains),
  };
}
...
async function cacheProject(cache: KVNamespace, project: ProjectRow) {
  await cache.put(
    ...
    JSON.stringify(cacheValue(project)),
...
export async function updateProject(
  ...
  if (parsed.data.name !== undefined)
    await repository.updateName(id, ownerId, parsed.data.name, now);
  if (parsed.data.allowed_domains !== undefined) {
    const allowedDomains = normalizeAllowedDomains(parsed.data.allowed_domains);
    await repository.updateAllowedDomains(
      id,
      ownerId,
