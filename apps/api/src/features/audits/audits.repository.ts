...
export class AuditsRepository {
  ...
  findOwnedProject(projectId: string, ownerId: string) {
    return this.db.prepare("SELECT * FROM projects WHERE id = ? AND owner_id = ? AND is_active = 1").bind(projectId, ownerId).first<ProjectRow>();
  }
  ...
  insert(audit: AuditInsert) {
    ...
    ).bind(audit.id, audit.project_id, audit.url, audit.ran_at, audit.performance, audit.accessibility, audit.best_practices, audit.seo, audit.agentic, audit.lcp_ms, audit.inp_ms, audit.cls, audit.raw_json).run();
  }
