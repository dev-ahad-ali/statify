...
function projectId(req: Request) {
  ...
  if (typeof value !== "string") throw new Error("Project id is required");
  return value;
}

export async function listAuditsController(req: Request, res: Response) {
  const project = await repository.findOwnedProject(
    projectId(req),
    requireUserId(req),
  );
  ...
  const limit = Number.isInteger(parsed)
    ...
    : 30;
  const rows = await repository.listOwned(
    project.id,
    requireUserId(req),
    limit,
  );
  const vitals = await repository.listFieldVitals(
    project.id,
    requireUserId(req),
  );
  const field = Object.fromEntries(
    vitals.results.map((vital) => [
      vital.name,
      {
        count: vital.count,
        mean: vital.count ? vital.total / vital.count : null,
      },
    ]),
  );
  return success(res, "Audits retrieved", {
    audits: rows.results.map(serializeAudit),
    field,
  });
}
...
export async function runAuditController(req: Request, res: Response) {
  ...
  if (!runtimeEnv.PSI_API_KEY)
    return failure(res, "PageSpeed API is not configured", 503);
  const key = `audit-run:${project.id}`;
  if (await runtimeEnv.CACHE.get(key))
    return failure(res, "This project was audited recently", 429);
  await runtimeEnv.CACHE.put(key, "1", { expirationTtl: 600 });
  try {
    await runFullAudit(repository, project, runtimeEnv.PSI_API_KEY);
    return success(res, "Audit completed", null, 201);
  } catch (error) {
    await runtimeEnv.CACHE.delete(key);
    console.error("manual audit failed", error);
    return failure(res, "Unable to run audit", 502);
  }
}
