...
const dashboardQuerySchema = z.object({
  range: z
    .enum(["today", "yesterday", "7d", "30d", "this_month", "12m", "all"])
    .default("7d"),
...
export function resolveDashboardRange(
  slug: z.infer<typeof dashboardQuerySchema>["range"],
  now = new Date(),
): DashboardRange {
...
export function parseDashboardQuery(
  ...
  return {
    range: resolveDashboardRange(range, now),
    pageView,
    locationView,
    deviceView,
...
export async function getDashboard(
  ...
  const hasFilters = Object.values(options.filters).some(
    (value) => value !== undefined,
  );
  const data = hasFilters
    ? await repository.fromEvents(projectId, options)
    : await repository.fromRollups(projectId, options);
  return {
    projectId,
    range: options.range,
    views: {
      pageView: options.pageView,
      locationView: options.locationView,
      deviceView: options.deviceView,
    },
    filtered: hasFilters,
    ...data,
  };
