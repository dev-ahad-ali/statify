...
export async function refreshController(req: Request, res: Response) {
  try {
    const result = await refresh(repository, readCookie(req, "refresh_token") ?? "");
    setAuthCookies(res, result.accessToken, result.refreshToken);
    return success(res, "Token refreshed", result.user);
  } catch (error) {
    if (error instanceof InvalidRefreshTokenError) return failure(res, "Invalid refresh token", 401);
    throw error;
  }
}
...
export async function resetController(req: Request, res: Response) {
  ...
  if (!parsed.success) return failure(res, "Invalid or expired reset token", 400);
  const reset = await resetPassword(repository, parsed.data.token, parsed.data.password);
...
export async function meController(req: Request, res: Response) {
  ...
  return success(res, "Authenticated user", { id: user.id, email: user.email, name: user.name });
}
