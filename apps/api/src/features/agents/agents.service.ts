...
export function parseAgentsQuery(
  ...
  if (!parsed.success)
    throw new AgentsValidationError(
      parsed.error.issues[0]?.message ?? "Invalid agents query",
    );
  return {
    range: resolveDashboardRange(parsed.data.range),
...
export async function getAgents(
  ...
  if (!(await repository.findOwnedProject(projectId, ownerId)))
    throw new AgentsNotFoundError("Project not found");
  return {
    projectId,
    range: options.range,
    vendor: options.vendor ?? null,
    ...(await repository.getAgents(projectId, options)),
  };
}
