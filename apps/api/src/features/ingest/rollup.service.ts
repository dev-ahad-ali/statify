...
function dateFor(event: QueuedEvent) {
  ...
  const timestamp =
    drift > 24 * 60 * 60 * 1000 ? event.receivedAt : event.timestamp;
...
function pageViewRollups(
  ...
  const statements: D1PreparedStatement[] = [
    upsert(
      ...
      db,
      [projectId, date, entries, entries],
    ),
    upsert(
      `INSERT OR IGNORE INTO ${ROLLUP_TABLES.visitors} (${ROLLUP_COLUMNS.visitors.projectId}, ${ROLLUP_COLUMNS.visitors.date}, ${ROLLUP_COLUMNS.visitors.visitorId}, ${ROLLUP_COLUMNS.visitors.firstSeenAt}) VALUES (?, ?, ?, ?)`,
      db,
      [projectId, date, event.visitorId, event.timestamp],
    ),
    upsert(
      `UPDATE ${stats} SET ${statsColumns.visitors} = ${statsColumns.visitors} + changes() WHERE ${statsColumns.projectId} = ? AND ${statsColumns.date} = ?`,
      db,
      [projectId, date],
    ),
    upsert(
      `INSERT OR IGNORE INTO ${ROLLUP_TABLES.sessions} (${ROLLUP_COLUMNS.sessions.projectId}, ${ROLLUP_COLUMNS.sessions.date}, ${ROLLUP_COLUMNS.sessions.sessionId}, ${ROLLUP_COLUMNS.sessions.firstSeenAt}) VALUES (?, ?, ?, ?)`,
      db,
      [projectId, date, event.sessionId, event.timestamp],
    ),
    upsert(
      `UPDATE ${stats} SET ${statsColumns.sessions} = ${statsColumns.sessions} + changes() WHERE ${statsColumns.projectId} = ? AND ${statsColumns.date} = ?`,
      db,
      [projectId, date],
    ),
    upsert(
      `INSERT INTO ${ROLLUP_TABLES.pages} (${ROLLUP_COLUMNS.pages.projectId}, ${ROLLUP_COLUMNS.pages.date}, ${ROLLUP_COLUMNS.pages.path}, ${ROLLUP_COLUMNS.pages.pageviews}, ${ROLLUP_COLUMNS.pages.entries}) VALUES (?, ?, ?, 1, ?) ON CONFLICT (${ROLLUP_COLUMNS.pages.projectId}, ${ROLLUP_COLUMNS.pages.date}, ${ROLLUP_COLUMNS.pages.path}) DO UPDATE SET ${ROLLUP_COLUMNS.pages.pageviews} = ${ROLLUP_COLUMNS.pages.pageviews} + 1, ${ROLLUP_COLUMNS.pages.entries} = ${ROLLUP_COLUMNS.pages.entries} + ?`,
      db,
      [projectId, date, path, entries, entries],
    ),
    upsert(
      `INSERT INTO ${ROLLUP_TABLES.referrers} (${ROLLUP_COLUMNS.referrers.projectId}, ${ROLLUP_COLUMNS.referrers.date}, ${ROLLUP_COLUMNS.referrers.referrer}, ${ROLLUP_COLUMNS.referrers.pageviews}) VALUES (?, ?, ?, 1) ON CONFLICT (${ROLLUP_COLUMNS.referrers.projectId}, ${ROLLUP_COLUMNS.referrers.date}, ${ROLLUP_COLUMNS.referrers.referrer}) DO UPDATE SET ${ROLLUP_COLUMNS.referrers.pageviews} = ${ROLLUP_COLUMNS.referrers.pageviews} + 1`,
      db,
      [projectId, date, referrer],
    ),
    upsert(
      `INSERT INTO ${ROLLUP_TABLES.countries} (${ROLLUP_COLUMNS.countries.projectId}, ${ROLLUP_COLUMNS.countries.date}, ${ROLLUP_COLUMNS.countries.countryCode}, ${ROLLUP_COLUMNS.countries.visitors}) VALUES (?, ?, ?, 1) ON CONFLICT (${ROLLUP_COLUMNS.countries.projectId}, ${ROLLUP_COLUMNS.countries.date}, ${ROLLUP_COLUMNS.countries.countryCode}) DO UPDATE SET ${ROLLUP_COLUMNS.countries.visitors} = ${ROLLUP_COLUMNS.countries.visitors} + 1`,
      db,
      [projectId, date, country],
    ),
    upsert(
      `INSERT INTO ${ROLLUP_TABLES.regions} (${ROLLUP_COLUMNS.regions.projectId}, ${ROLLUP_COLUMNS.regions.date}, ${ROLLUP_COLUMNS.regions.countryCode}, ${ROLLUP_COLUMNS.regions.region}, ${ROLLUP_COLUMNS.regions.visitors}) VALUES (?, ?, ?, ?, 1) ON CONFLICT (${ROLLUP_COLUMNS.regions.projectId}, ${ROLLUP_COLUMNS.regions.date}, ${ROLLUP_COLUMNS.regions.countryCode}, ${ROLLUP_COLUMNS.regions.region}) DO UPDATE SET ${ROLLUP_COLUMNS.regions.visitors} = ${ROLLUP_COLUMNS.regions.visitors} + 1`,
      db,
      [projectId, date, country, region],
    ),
    upsert(
      `INSERT INTO ${ROLLUP_TABLES.cities} (${ROLLUP_COLUMNS.cities.projectId}, ${ROLLUP_COLUMNS.cities.date}, ${ROLLUP_COLUMNS.cities.countryCode}, ${ROLLUP_COLUMNS.cities.region}, ${ROLLUP_COLUMNS.cities.city}, ${ROLLUP_COLUMNS.cities.visitors}) VALUES (?, ?, ?, ?, ?, 1) ON CONFLICT (${ROLLUP_COLUMNS.cities.projectId}, ${ROLLUP_COLUMNS.cities.date}, ${ROLLUP_COLUMNS.cities.countryCode}, ${ROLLUP_COLUMNS.cities.region}, ${ROLLUP_COLUMNS.cities.city}) DO UPDATE SET ${ROLLUP_COLUMNS.cities.visitors} = ${ROLLUP_COLUMNS.cities.visitors} + 1`,
      db,
      [projectId, date, country, region, city],
    ),
    upsert(
      `INSERT INTO ${ROLLUP_TABLES.browsers} (${ROLLUP_COLUMNS.browsers.projectId}, ${ROLLUP_COLUMNS.browsers.date}, ${ROLLUP_COLUMNS.browsers.browser}, ${ROLLUP_COLUMNS.browsers.visitors}) VALUES (?, ?, ?, 1) ON CONFLICT (${ROLLUP_COLUMNS.browsers.projectId}, ${ROLLUP_COLUMNS.browsers.date}, ${ROLLUP_COLUMNS.browsers.browser}) DO UPDATE SET ${ROLLUP_COLUMNS.browsers.visitors} = ${ROLLUP_COLUMNS.browsers.visitors} + 1`,
      db,
      [projectId, date, browser],
    ),
    upsert(
      `INSERT INTO ${ROLLUP_TABLES.os} (${ROLLUP_COLUMNS.os.projectId}, ${ROLLUP_COLUMNS.os.date}, ${ROLLUP_COLUMNS.os.os}, ${ROLLUP_COLUMNS.os.visitors}) VALUES (?, ?, ?, 1) ON CONFLICT (${ROLLUP_COLUMNS.os.projectId}, ${ROLLUP_COLUMNS.os.date}, ${ROLLUP_COLUMNS.os.os}) DO UPDATE SET ${ROLLUP_COLUMNS.os.visitors} = ${ROLLUP_COLUMNS.os.visitors} + 1`,
      db,
      [projectId, date, os],
    ),
    upsert(
      `INSERT INTO ${ROLLUP_TABLES.devices} (${ROLLUP_COLUMNS.devices.projectId}, ${ROLLUP_COLUMNS.devices.date}, ${ROLLUP_COLUMNS.devices.device}, ${ROLLUP_COLUMNS.devices.visitors}) VALUES (?, ?, ?, 1) ON CONFLICT (${ROLLUP_COLUMNS.devices.projectId}, ${ROLLUP_COLUMNS.devices.date}, ${ROLLUP_COLUMNS.devices.device}) DO UPDATE SET ${ROLLUP_COLUMNS.devices.visitors} = ${ROLLUP_COLUMNS.devices.visitors} + 1`,
      db,
      [projectId, date, device],
    ),
  ];
  ...
  return [
    upsert(
      ...
      [
        ...
        date,
        event.agentCategory,
        sqlValue(event.agentVendor),
        sqlValue(event.agentHarness),
        sqlValue(event.agentConfidence),
      ],
    ),
  ];
}

function vitalRollup(
  db: D1Database,
  projectId: string,
  event: QueuedEvent,
): D1PreparedStatement[] {
  if (
    ...
    !["LCP", "INP", "CLS"].includes(event.vitalName)
  )
    return [];
  const date = dateFor(event);
  return [
    upsert(
      ...
      [
        ...
        event.vitalName,
        event.vitalValue,
        JSON.stringify([event.vitalValue]),
        event.vitalValue,
        JSON.stringify([event.vitalValue]),
      ],
    ),
...
export async function writeEventBatch(
  ...
  const statements = events.flatMap((event) => [
    ...
    ...(event.type === "page_view"
      ? pageViewRollups(db, projectId, event)
      : []),
