...
import { getAudits, runAudit, type Audit, type AuditCheck, type FieldVital } from "@/lib/audits";
...
export function AuditsClient({ demo: demoProp }: { demo?: boolean } = {}) {
  ...
  const [field, setField] = useState<Partial<Record<"LCP" | "INP" | "CLS", FieldVital>>>({});
  ...
  const load = useCallback(async () => { if (!selected || demo) return; setError(null); const result = await getAudits(selected.id); if (result.error || !result.data) setError(result.error ?? "Unable to load audits"); else { setAudits(result.data.audits); setField(result.data.field); } }, [demo, selected]);
