    ...
    return (
      <Card>
        <CardHeader>
          ...
          <CardDescription>
            Create a project from the dashboard before opening settings.
          </CardDescription>
        </CardHeader>
      </Card>
    );
...
function SettingsView({
  ...
  async function saveName() {
    ...
    setSavingName(false);
    if (result.error || !result.data)
      toast.error(result.error ?? "Unable to save project name");
    else {
  ...
  async function saveDomains(nextDomains: string[]) {
    ...
    if (result.error || !result.data)
      toast.error(result.error ?? "Unable to save allowed domains");
    else {
      onProjectChange(result.data);
      setDomainInput("");
  ...
  async function addDomain(event: React.FormEvent<HTMLFormElement>) {
    ...
    if (domain === project.domain || project.allowed_domains.includes(domain)) {
      toast.error("That domain is already listed");
      return;
    }
  ...
  async function rotate() {
    ...
    setRotating(false);
    if (result.error || !result.data)
      toast.error(result.error ?? "Unable to rotate API key");
    else {
      onProjectChange(result.data);
  ...
  async function removeDomain(domain: string) {
    await saveDomains(
      project.allowed_domains.filter((item) => item !== domain),
    );
  }
        ...
        <TabsContent value="general">
          <Card>
            <CardHeader>
              ...
              <CardDescription>
                Change the name shown in your dashboard. The project domain
                cannot be changed.
              </CardDescription>
            </CardHeader>
            <CardContent className="space-y-5">
              ...
              <label className="grid gap-2 text-sm font-medium">
                ...
                <input
                  ...
                  readOnly
                  className="h-10 rounded-md border bg-muted px-3 font-normal text-muted-foreground"
                />
              </label>
              <Button
        ...
        <TabsContent value="domains">
          <Card>
            <CardHeader>
              ...
              <CardDescription>
                Only these browser origins can send events for this project.
              </CardDescription>
            </CardHeader>
            <CardContent className="space-y-5">
              <div className="space-y-2">
                <DomainRow domain={project.domain} primary />
                  ...
                  {project.allowed_domains.length === 0 ? (
                    ...
                    project.allowed_domains.map((domain) => (
                      <DomainRow
                        key={domain}
                        domain={domain}
                        onRemove={() => void removeDomain(domain)}
                      />
                    ))
                  )}
                </div>
              ...
              >
                <div className="flex-1">
                  ...
                  >
                    You can paste a URL. The protocol and path are removed
                    automatically.
                  </p>
                </div>
        ...
        <TabsContent value="danger">
          <Card className="border-destructive/50">
            <CardHeader>
              ...
              <CardDescription>
                This removes the project from your account and stops accepting
                events. Existing data will remain in the database until
                retention cleanup.
              </CardDescription>
            </CardHeader>
            <CardContent>
      ...
      <Dialog open={rotating} onOpenChange={setRotating}>
        <DialogContent>
          <DialogHeader>
            ...
            <DialogDescription>
              The old key will stop accepting events. Update your sites with the
              new key after rotation.
            </DialogDescription>
          </DialogHeader>
          <DialogFooter>
            <Button variant="outline" onClick={() => setRotating(false)}>
              Cancel
            </Button>
...
function InstallTab({ project }: { project: WebProject }) {
  ...
  const code = useMemo(
    () => selected.code.replaceAll("YOUR_API_KEY", project.api_key),
    [project.api_key, selected.code],
  );
  useEffect(() => {
    let active = true;
  ...
  return (
    <Card>
      <CardHeader>
        <CardTitle>Install Statify</CardTitle>
        <CardDescription>
          Choose your framework and add the generated snippet to{" "}
          {project.domain}.
        </CardDescription>
      </CardHeader>
      <CardContent className="space-y-5">
        <div className="flex flex-wrap gap-2">
          {installSnippets.map((snippet) => (
            <Button
              key={snippet.label}
              variant={snippet.label === selected.label ? "default" : "outline"}
              size="sm"
        ...
        <div className="relative overflow-hidden rounded-lg bg-[#24292e]">
          ...
          {html ? (
            <div
              className="overflow-x-auto p-4 text-sm [&_pre]:m-0"
              dangerouslySetInnerHTML={{ __html: html }}
            />
          ) : (
            <pre className="overflow-x-auto p-4 text-sm text-white">{code}</pre>
          )}
        </div>
      </CardContent>
    </Card>
    ...
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent>
        <DialogHeader>
          ...
          <DialogDescription>
            Type <strong>{project.domain}</strong> to confirm. This cannot be
            undone.
          </DialogDescription>
        </DialogHeader>
        <input
        ...
        <DialogFooter>
          ...
          <Button
            variant="destructive"
            disabled={confirmation !== project.domain || deleting}
            onClick={() => void remove()}
          >
