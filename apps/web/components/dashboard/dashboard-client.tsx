  ...
  return (
    <div className="space-y-8">
      <header className="flex flex-col gap-4 border-b pb-6 sm:flex-row sm:items-center sm:justify-between">
        ...
        <div className="flex flex-wrap items-center gap-2"><ProjectMenu demo={demo} projects={projects} selected={activeProject} onSelect={(project) => updateUrl(router, pathname, new URLSearchParams(searchParams.toString()), "project", project.id)} onCreated={(project) => { setProjects((current) => [...current, project]); updateUrl(router, pathname, new URLSearchParams(searchParams.toString()), "project", project.id); }} /><RangeMenu range={range} onSelect={(value) => updateUrl(router, pathname, new URLSearchParams(searchParams.toString()), "range", value)} />{activeProject && <><Button variant="outline" asChild><Link href={demo ? "/demo/agents" : `/dashboard/agents?project=${encodeURIComponent(activeProject.id)}`}>Agents</Link></Button><Button variant="outline" asChild><Link href={demo ? "/demo/audits" : `/dashboard/audits?project=${encodeURIComponent(activeProject.id)}`}>Audits</Link></Button></>}{demo ? <Button variant="outline" asChild><Link href="/">Exit demo</Link></Button> : <>{selected && <Button variant="outline" asChild><Link href={`/settings?project=${encodeURIComponent(selected.id)}`}>Settings</Link></Button>}<LogoutButton /></>}</div>
      </header>
